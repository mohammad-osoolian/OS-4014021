...
void* cal_element(void* arg){
    for (int t = 0; t < m-1; t++)
    {
        ...
        // printmatrix();
        // printf("debug\n");
        pthread_mutex_lock(&freel);
        if (freec == 1)
            resetc = limit;
        freec--;
        pthread_mutex_unlock(&freel);
        while (freec)
            continue;
        ...
        pthread_mutex_lock(&resetl);
        if(resetc == 1)
            freec = limit;
        resetc--;
        pthread_mutex_unlock(&resetl);
        while (resetc)
            continue;
...
int main(){
    ...
    limit = n * n;
    init();
    pthread_mutex_init(&freel, NULL);
    pthread_mutex_init(&resetl, NULL);
